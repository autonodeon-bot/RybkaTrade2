...
import { PositionType, Trade, Timeframe } from '../types';

const PAIRS = ['BTC_USDT', 'ETH_USDT', 'SOL_USDT', 'TON_USDT', 'DOGE_USDT'];
const COMMISSION_RATE = 0.001; // 0.1%

// This function runs every minute automatically on Vercel
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    for (const pair of PAIRS) {
        ...
        updatedTrades = updatedTrades.map(trade => {
            ...
            if (shouldClose) {
                ...
                updatedPortfolio.balance += finalPnL; // Realize PnL into Balance (Balance is Total Cash)
    ...
    // Standard Accounting: Equity = Balance + Unrealized PnL
    ...
    return res.status(200).json({ success: true, timestamp: Date.now(), equity: updatedPortfolio.equity });
}